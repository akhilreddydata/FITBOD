...
def read_files(file1, file2, file3):
    """ Load the input files into pandas dataframes
    """
...
def merge_user_alias_dataframe(users_df, alias_df):
    """ Merge the users dataframe with the aliases dataframe using
        user_id as common parameter
    """
...
def merge_final_dataframe(merged_df, events_df):
    """ Merge the result with the events dataframes
...
def Aggregate_by_feature(final_df):
    """ Aggregate the data by feature_key, feature_value to get the final output
    """
